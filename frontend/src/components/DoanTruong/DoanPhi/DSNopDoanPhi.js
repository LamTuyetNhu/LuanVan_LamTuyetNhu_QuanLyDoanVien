...
const NopDoanPhi = (props) => {
  ...
  const fetchDSNopDoanPhi = async () => {
    try {
      let res = await LayDSNopDoanPhi(IDDoanPhi);
      console.log("API Response:", res.data); // A
      if (res.status === 200) {
  ...
  return (
    <>
      <div className="container-fluid app__content">
        <h5 className="text-center">Danh Sách Nộp Đoàn Phí - {TenDP}</h5>
            ...
            <tbody id="myTable">
              ...
              {DSNopDoanPhi && DSNopDoanPhi.length === 0 && (
                <tr className="tablenone">
                  <td className="tablenone">Chưa có có chi đoàn phí!</td>
                </tr>
              )}
            </tbody>
          </table>
