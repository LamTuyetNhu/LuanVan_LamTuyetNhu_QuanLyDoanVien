...
import { layTieuChiDoanVien } from "../../../services/apiService";
...
function ThongTinDiemThi() {
  ...
  const IDTieuChiDV = localStorage.getItem("IDTieuChiDV");
  ...
  const fetchData = async () => {
    try {
      ...
      const ND = res.data.dataTC[0].NoiDungTC;
      console.log(ND);
      setEditorData(ND);
    } catch (error) {
  ...
  const handleUpdate = async () => {
    try {
      const update = await axios.post(
        `http://localhost:8080/api/CapNhatTieuChiDoanVien/${IDTieuChiDV}`,
        {
          NoiDungTC: editorData,
        }
      );
      ...
      <div className="tableCK">
        ...
        <div className="update row">
          ...
          <button className="allcus-button" type="submit">
            <NavLink
              to={`/DaiHocCanTho/TieuChiDanhGiaChiDoan`}
              className="navlink"
            >
